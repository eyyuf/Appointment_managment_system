...
const checkConflict = async (leaderId, date, startTime, endTime, excludeId = null) => {
  ...
  return existing.find((appointment) => timesOverlap(startTime, endTime, appointment.startTime, appointment.endTime));
};
...
const secretaryMarkUnderReview = async (id, secretaryId) => {
  ...
  if (!secretary || secretary.role !== 'SECRETARY') {
    throw Object.assign(new Error('Only secretaries can review'), { statusCode: 403 });
  }
  if (appointment.status !== 'PENDING') {
    throw Object.assign(new Error('Appointment is not pending'), { statusCode: 400 });
  }
...
const secretaryForward = async (id, secretaryId, { leaderId, note, newDate, newStartTime, newEndTime }) => {
  ...
  if (conflict) {
    throw Object.assign(new Error(`Time conflict: "${conflict.title}"`), { statusCode: 409 });
  }
...
const rejectAppointment = async (id, userId, reason) => {
  ...
  if (!user) throw Object.assign(new Error('Forbidden'), { statusCode: 403 });
  ...
  if (!canReject) {
    throw Object.assign(new Error('You cannot reject this appointment in its current state'), { statusCode: 403 });
  }
...
const requestReschedule = async (appointmentId, requestedById, data) => {
  ...
  if (appointment.leaderId) {
    ...
    if (conflict) {
      throw Object.assign(new Error('New time slot conflicts with existing appointment'), { statusCode: 409 });
    }
  }
...
const getAppointments = async (user, filters = {}) => {
  ...
  if (user.role === 'STUDENT') {
    ...
    where.OR = [{ secretaryId: user.id }, { status: 'PENDING', targetDepartment: user.department }];
  ...
  return { appointments, total, page: Number(page), limit: Number(limit), totalPages: Math.ceil(total / Number(limit)) };
};
...
const getDepartments = async () => {
  ...
  return departments.map((department) => department.department).filter(Boolean);
};
